...
const AstaPlugin = Object.freeze({
  command: "تفكيك",
  description: "فعالية تفكيك الكلمات",
  elite: "off",
  group: false,
  prv: false,
  lock: "off",
...
async function startRound(sock, chatId, total, current) {
  ...
  const answer = splitWord(word);

  await sock.sendMessage(chatId, {
    text: `*❊┇━━─≪•◦🕸◦•≫─━━┇❊*
*「⛓‍💥┊فعالية التفكيك┊⛓‍💥」*
*يقوم المقدم باختيار اسم شخصية و اول من يفكك الشخصية هو الرابح*
*❊┇━━─≪•◦🕸◦•≫─━━┇❊*
...
*_【𝑱. 𝑨. 𝑭. 𝑻◈|🕸|◈𝑬𝑺𝑷𝑨𝑫𝑨】🝳_*`
  ...
  const listener = async ({ messages }) => {
    ...
    if (normalize(body) === normalize(game.answer)) {
